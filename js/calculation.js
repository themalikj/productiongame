...
//get player type
  ...
  consequenceRef.once("value")
    .then(function(snapshot) {
      sessionStorage.setItem('playerType', snapshot.child("player").val());
    });

    playerType = sessionStorage.getItem('playerType');
...
eventRef.once("value")
  .then(function(snapshot) {
    sessionStorage.setItem('eventSetOps', snapshot.child("SetOperatives/currentEvent").val());
    sessionStorage.setItem('eventCreative', snapshot.child("Creative/currentEvent").val());
    sessionStorage.setItem('eventBudgetary', snapshot.child("Budgetary/currentEvent").val());
  });

eventSetOps = sessionStorage.getItem('eventSetOps');
eventCreative = sessionStorage.getItem('eventCreative');
eventBudgetary = sessionStorage.getItem('eventBudgetary');
...
var sortingRefSO = ref.child("Games/" + gameKey + "/Events/" + eventSetOps);
 sortingRefSO.once("value")
   .then(function(snapshot) {
    sessionStorage.setItem('event1array', JSON.stringify((snapshot.val().choice)));
  });

  var sortingRefB = ref.child("Games/" + gameKey + "/Events/" + eventBudgetary);
   sortingRefB.once("value")
     .then(function(snapshot) {
      sessionStorage.setItem('event1array', JSON.stringify((snapshot.val().choice)));
    });

    var sortingRefC = ref.child("Games/" + gameKey + "/Events/" + eventCreative);
     sortingRefC.once("value")
       .then(function(snapshot) {
        sessionStorage.setItem('event1array', JSON.stringify((snapshot.val().choice)));
      });

setTimeout(restOfIt(), 3000, gameKey, pace);
}
...
event1array = JSON.parse(sessionStorage.getItem('event1array'));
event2array = JSON.parse(sessionStorage.getItem('event2array'));
event3array = JSON.parse(sessionStorage.getItem('event3array'));
  ...
  currentRef.once("value")
    .then(function(snapshot) {
     sessionStorage.setItem('currentBudgetary', snapshot.child("Budgetary/temperment").val());
     sessionStorage.setItem('currentCreative', snapshot.child("Creative/temperment").val());
     sessionStorage.setItem('currentSetOps', snapshot.child("SetOperatives/temperment").val());
    });

    currentBudgetary = sessionStorage.getItem('currentBudgetary');
    currentCreative = sessionStorage.getItem('currentCreative');
    currentSetOps = sessionStorage.getItem('currentSetOps');
